...
class PlotUpdater:
    def __init__(self, plot_graph, main_window):
        ...
        # Define a gradient for the muddy fill
        self.gradient = QLinearGradient(0, 0, 0, 1)  # Vertical gradient
        self.gradient.setColorAt(0, QColor(255, 0, 0))  # Red at the top
        self.gradient.setColorAt(0.5, QColor(0, 255, 0))  # Green in the middle
        self.gradient.setColorAt(1, QColor(0, 0, 255))  # Blue at the bottom

        # Wrap the gradient in a QBrush
        gradient_brush = QBrush(self.gradient)
        self.muddy_fill.setBrush(gradient_brush)
    ...
    def update_plot(self, depth, temperature):
        ...
        # Update the gradient to match the plot's Y-axis
        y_min, y_max = self.plot_graph.getViewBox().viewRange()[1]  # Get Y-axis range
        self.gradient.setStart(0, y_max)  # Top of the plot
        self.gradient.setFinalStop(0, y_min)  # Bottom of the plot

        # Reapply the gradient brush
        gradient_brush = QBrush(self.gradient)
        self.muddy_fill.setBrush(gradient_brush)
